...
1. **The `grove-review-policy` block gains a `scope` key** with two
   ...
   - **`scoped`** — the check governs **only what is declared into the
     ...
     - files under the policy's `artifact_dirs` (the governed corpus);
     - code belonging to a package that **opted in via a test-deps
       ledger** (nearest-ancestor `test-deps.md`, `adr-0006` dec 4);
     - **the check's own policy carriers** — the discovered
       reviewer-declaration directory's files, the review-policy file
       itself, and every `test-deps.md` — so the gate's own inputs are
       never ungoverned, in either mode.
...
- **AC2 (jurisdiction).** In `scoped` mode, a changed file outside
  scope generates zero owed pairs and zero reasons — silent, never a
  listed exemption row.
- **AC3 (no softening).** In `scoped` mode, an in-scope file of
  unclaimed type owes the full review set — INV7 intact within
  jurisdiction.
- **AC4 (the gate governs itself).** In `scoped` mode, the
  reviewer-declaration files, the review-policy file, and every ledger
  are in scope in both modes.
...
checkable. Gated pending the decision-adversary pass and the
